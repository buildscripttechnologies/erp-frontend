...
const StockRegister = () => {
  ...
  const [overallInventoryValue, setOverallTotalAmount] = useState();
  const [expandedStockId, setExpandedStockId] = useState(null);
  const [filters, setFilters] = useState({
    type: "",
    uom: "",
  ...
  const fetchstocks = async (page = 1, limit = pagination.limit) => {
    ...
    try {
      ...
      if (res.data.status == 200) {
        setOverallTotalAmount(res.data.overallInventoryValue || "");
        setstocks(res.data.data || []);
        setPagination({
          currentPage: res.data.currentPage,
          totalPages: res.data.totalPages,
  ...
  return (
    <div className="relative p-2 mt-4 md:px-4 max-w-[99vw] mx-auto overflow-x-hidden">
      <h2 className="text-xl sm:text-2xl font-bold mb-4 text-primary">
        Stock <span className="text-primary">({stocks.length})</span>
      </h2>
      ...
      <div className="flex justify-end">
        <p className="text-red-600 font-bold text-sm">
          {overallInventoryValue ? `TotalAmount : ₹${overallInventoryValue}` : ""}
        </p>
      </div>
      <div className="overflow-x-auto rounded border border-primary shadow-sm">
        <table className="min-w-full text-[11px]">
          <thead className="bg-primary text-secondary text-left whitespace-nowrap">
            <tr>
              ...
              {/* <th className="px-2 py-1.5">GST Amount</th> */}
              <th className="px-2 py-1.5">Total Amount</th>
              <th className="px-2 py-1.5">Issued Qty</th>
              <th className="px-2 py-1.5">Issued Amount</th>
              <th className="px-2 py-1.5">Damage Loss</th>
              <th className="px-2 py-1.5">Inventory Value</th>
          ...
          <tbody>
            {loading ? (
              <TableSkeleton rows={pagination.limit} columns={Array(16).fill({})} />
            ) : (
              <>
                {stocks.map((stock, index) => (
                  <tr
                    key={index}
                    className={`border-t text-[11px] border-primary whitespace-nowrap
                ${stock.availableQty < stock.moq
              ...
              `}
                  >
                    ...
                    {/* TYPE */}
                    <td className="px-2 py-1.5 border-r border-primary text-left">
                      <span className="px-2 py-0.5 text-[11px] font-semibold bg-gray-200 text-gray-800 rounded">
                        {stock.type || "-"}
                      </span>
                    </td>
                    ...
                    {/* ITEM NAME */}
                    <td className="px-2 py-1.5 border-r border-primary text-left max-w-[220px]">
                      <span
                        className="px-2 py-0.5 text-[11px] font-semibold bg-slate-100 text-slate-900 rounded truncate block"
                    ...
                    {/* DESCRIPTION */}
                    <td className="px-2 py-1.5 border-r border-primary text-left max-w-[200px]">
                      <span
                        className="px-2 py-0.5 text-[11px] bg-gray-50 text-gray-700 rounded truncate block"
                    ...
                    <td className="px-2 py-1.5 border-r border-primary text-left">
                      {stock.stockUOM ? (
                        <span className="px-2 py-0.5 text-[10px] font-bold bg-primary text-white rounded">
                          {stock.stockUOM}
                        </span>
                      ) : (
                        "-"
                    ...
                    {/* STOCK QTY */}
                    <td className="px-2 py-1.5 border-r border-primary text-left">
                      <span className="px-2 py-0.5 text-[11px] font-bold bg-gray-100 text-gray-800 rounded">
                        {Number(stock.stockQty || 0).toFixed(2)}
                      </span>
                    </td>

                    {/* AVAILABLE */}
                    <td className="px-2 py-1.5 border-r border-primary text-left">
                      <span className="px-2 py-0.5 text-[11px] font-bold bg-green-100 text-green-800 rounded">
                        {Number(stock.availableQty || 0).toFixed(2)}
                      </span>
                    </td>

                    {/* DAMAGED */}
                    <td className="px-2 py-1.5 border-r border-primary text-left">
                      <span className="px-2 py-0.5 text-[11px] font-semibold bg-red-100 text-red-700 rounded">
                        {Number(stock.damagedQty || 0).toFixed(2)}
                      </span>
                    </td>

                    {/* MOQ */}
                    <td className="px-2 py-1.5 border-r border-primary text-left">
                      <span className="px-2 py-0.5 text-[11px] font-semibold bg-yellow-100 text-yellow-800 rounded">
                        {Number(stock.moq || 0).toFixed(2)}
                      </span>
                    </td>

                    {/* GST */}
                    <td className="px-2 py-1.5 border-r border-primary text-left">
                      <span className="px-2 py-0.5 text-[11px] font-semibold bg-grey-900 text-grey-800 rounded">
                        {Number(stock.gst || 0).toFixed(2)}%
                      </span>
                    </td>
                    ...
                    {/* NET */}
                    {/* <td className="px-2 border-r border-primary">
                      {Number(stock.baseAmount || 0).toFixed(2)}
                    </td> */}

                    {/* GST AMT */}
                    {/* <td className="px-2 border-r border-primary">
                      {Number(stock.gstAmount || 0).toFixed(2)}
                    </td> */}

                    {/* TOTAL (GRN VALUE) */}
                    <td className="px-2 py-1.5 border-r border-primary text-left">
                      <span className="px-2 py-0.5 text-[11px] font-semibold bg-blue-100 text-blue-800 rounded">
                        ₹{Number(stock.grnTotalAmount || 0).toLocaleString("en-IN")}
                      </span>
                    </td>

                    {/* ISSUED QTY */}
                    <td className="px-2 py-1.5 border-r border-primary text-left">
                      <span className="px-2 py-0.5 text-[11px] font-semibold bg-orange-100 text-orange-800 rounded">
                        {Number(stock.issuedQty || 0).toFixed(2)}
                      </span>
                    </td>

                    {/* ISSUED AMOUNT */}
                    <td className="px-2 py-1.5 border-r border-primary text-left">
                      <span className="px-2 py-0.5 text-[11px] font-semibold bg-orange-100 text-orange-900 rounded">
                        ₹{Number(stock.issuedAmount || 0).toLocaleString("en-IN")}
                      </span>
                    </td>

                    {/* DAMAGE LOSS */}
                    <td className="px-2 py-1.5 border-r border-primary text-left">
                      <span className="px-2 py-0.5 text-[11px] font-semibold bg-red-100 text-red-800 rounded">
                        ₹{Number(stock.damageAmount || 0).toLocaleString("en-IN")}
                      </span>
                    </td>

                    {/* INVENTORY VALUE */}
                    <td className="px-2 py-1.5 border-r border-primary text-left">
                      <span className="px-2 py-0.5 text-[11px] font-bold bg-green-100 text-green-900 rounded">
                        ₹{Number(stock.inventoryTotalAmount || 0).toLocaleString("en-IN")}
                      </span>
                    </td>
