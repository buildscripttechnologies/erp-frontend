...
const UpdateMI = ({ MIData, onClose, onUpdated }) => {
  ...
  const [itemDetails, setItemDetails] = useState(MIData.itemDetails);
  ...
  useEffect(() => {
    if (MIData?.itemDetails?.length) {
      const normalized = MIData.itemDetails.map((item) => {
        // if it's an object (with _id), convert it to string
        if (item.assignee && typeof item.assignee === "object") {
          return { ...item, assignee: item.assignee._id };
        }
        // if it's already a string or null, leave it
        return item;
      });
