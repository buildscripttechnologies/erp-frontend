...
const EditRoleModal = ({ role, onClose, onUpdated }) => {
  ...
  const handleUpdate = async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.patch(`/roles/update-role/${role._id}`, { name });
      if (res.status === 200) {
  ...
  return (
    <div className="fixed  inset-0 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white w-[92vw] max-w-md rounded-lg shadow-lg p-6 relative border-2 border-[#d8b76a]">
        ...
        <h2 className="text-lg font-semibold text-[#d8b76a] mb-4">Edit Role</h2>
        ...
        <div className="flex justify-end mt-4">
          <button
            ...
            disabled={loading}
            className="bg-[#d8b76a] text-black px-4 py-2 rounded items-center justify-center flex hover:bg-[#d8b76a]/80 font-semibold cursor-pointer"
          >
            {loading ? (
              <>
